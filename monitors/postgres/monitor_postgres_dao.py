...
def createTablesBase(conn):
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS lmp_monitor_postgres_dataT
        (
...
def insertMeasure(cursor, queryData):
    cursor.execute(
    """
    INSERT INTO lmp_monitor_postgres_dataT(exectime, 
          conns,
          locks
    ) 
    VALUES (?,
...
def insertMeasureFail(cursor, failmsg):
    cursor.execute(
    """
    INSERT INTO lmp_monitor_postgres_dataT(exectime, fail) 
    VALUES (?, ?);
    """
